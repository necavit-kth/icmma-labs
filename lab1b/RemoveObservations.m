function D = RemoveObservations(D,options)
    %-------------------------------------------------------
    % Function RemoveObservations(DATA,options)
    ...
    fprintf('\nNumber of removed observations (Missing Values): %5i\n', sum(miss_idx));
    ...
    if k>0
        ...
        DATA(mcrmv_idx,:)=[];
        fprintf('\nNumber of removed observations (Mode Choice %1i): %5i\n',[i sum(mcrmv_idx)]);
    end
    %
    % Remove Year  
        % This part removes all data observations in DATA relating to
    ...
    if k>0       
        ...
        DATA(yrmv_idx, :) = [];
        fprintf('\nNumber of removed observations (Year %4i): %5i\n',[i sum(yrmv_idx)]);
    end
    %
    % Remove Sex
        % This part removes all data observations in DATA relating to
    ...
    if k>0
        ...
        DATA(srmv_idx,:)=[];
        fprintf('\nNumber of removed observations (Sex %1i): %5i\n',[i sum(srmv_idx)]);
    end    
    %
    % Short dataset
    ...
    fprintf('\nNumber of individuals: %4i \n', nind);
    fprintf('\nNumber of observations per person: %4.2f \n', Nob/nind);

    %Store modified observations in structure
     D = struct('Name',D_IDX,'Data',DATA);   
end
